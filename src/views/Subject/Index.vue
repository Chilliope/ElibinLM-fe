<template>
    <div class="flex flex-col lg:justify-between lg:flex-row-reverse">
      <div class="flex">
        ...
        >/
        <router-link to="/subjek/1" class="capitalize text-blue-500">subjek</router-link>/
      </div>
      <div>
    ...
    <div class="flex flex-col items-center">
      ...
      >
        ...
        >
          ...
          <small class="text-slate-600">Tambah Subjek</small>
        </router-link>
      </div>
      ...
      <table class="my-4 table-auto w-full text-center">
        ...
        <tbody>
          <tr class="bg-white border-b h-14" v-for="subject in subject" :key="subject.id">
            <td class="text-xs lg:text-base">{{ subject.subject }}</td>
            <td class="flex gap-2 justify-center items-center h-14">
              <router-link
                :to="'/edit-subjek/' + subject.id"
  ...
  const { subject, getSubject, totalPage, count, destroy } = useSubject();
  ...
  watch(
    () => route.params.page,
    () => {
      getSubject(search.value)
    }
  );
  
  watch(search, (newSearch) => {
    getSubject(newSearch); // Fetch data in real-time as the search input changes
  });
