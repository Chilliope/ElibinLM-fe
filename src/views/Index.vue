<template>
    <div ref="overlay" class="bg-black w-full h-screen fixed bg-opacity-50 hidden z-10"></div>
    <nav class="w-full h-14 bg-white border-b flex justify-between items-center lg:h-16 fixed z-10">
        <div class="ml-6">
    ...
    <div ref="dropdownEl" class="bg-white w-44 h-max rounded-xl shadow-sm absolute right-6 top-16 overflow-hidden hidden">
        <ul class="w-full flex flex-col">
            <li v-for="item in dropdownItem" :key="item.name">
                <router-link :to="item.href" class=" w-full flex border-b px-3 py-3 items-center gap-3 hover:bg-gray-100 hover:duration-150 capitalize"><i :class="item.icon + ' w-4'"></i>{{ item.name }}</router-link>
            </li>
            <li>
                <button class="w-full flex border-b px-3 py-3 items-center gap-3 text-red-500 hover:bg-red-500 hover:text-white hover:duration-150"><i class="fa-solid fa-arrow-right-from-bracket"></i>Sign Out</button>
            </li>
        </ul>
    </div>
    <div class="flex flex-row">
        <aside ref="sidebar" class="fixed top-0 bg-white w-2/3 h-screen hidden lg:block lg:w-1/5 sidebarIsOpened z-10">
            <div class="font-medium px-3 py-2 border-b flex flex-row justify-between">
                ...
                <div>
                    <button @click="closeSidebar()" class="bg-slate-100 w-6 h-6 text-slate-800 rounded-full text-center hover:bg-slate-200 hover:duration-150 lg:hidden"><i class="fa-solid fa-x"></i></button>
                </div>
            </div>
            <ul class="px-3 mt-8 lg:mt-3 flex flex-col gap-6">
                <li v-for="item in sideItem" :key="item.name">
                    <router-link :to="item.href" class="px-3 py-1 rounded-xl w-full flex items-center capitalize hover:bg-blue-100 hover:duration-150" exact-active-class="bg-blue-500 hover:bg-blue-400 text-white" ><i :class="item.icon + ' w-7'"></i> {{ item.name }}</router-link>
                </li>
            </ul>
        </aside>
