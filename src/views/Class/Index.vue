<template>
    <div class="flex flex-col lg:justify-between lg:flex-row-reverse">
      <div class="flex">
        ...
        >/
        <router-link to="/kelas" class="capitalize text-blue-500">kelas</router-link>/
      </div>
      <div>
      ...
      <table class="my-4 table-auto w-full text-center">
        ...
        <tbody>
          <tr class="bg-white border-b h-14" v-for="item in 10" :key="item">
            <td class="text-xs lg:text-base">XII PPLG B</td>
            <td class="flex gap-2 justify-center items-center h-14">
              <router-link to="/edit-kelas/1" class="bg-yellow-500 text-white rounded-lg w-max h-max px-2 py-1 lg:px-3 lg:py-2 text-base lg:text-xl hover:bg-yellow-400 hover:duration-150"><i class="fa-regular fa-pen-to-square"></i></router-link>
              <form>
