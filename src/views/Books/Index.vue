...
const { getBook, book, destroy, totalPage, count } = useBook();
const { add } = useGate();
...
watch(
  () => route.params.page,
  () => {
    getBook(search.value); // Reload books when the page parameter changes
  }
);
...
onMounted(() => {
  getBook(search.value); // Load books on initial mount
});
