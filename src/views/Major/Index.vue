<template>
  <div class="flex flex-col lg:justify-between lg:flex-row-reverse">
    <div class="flex">
      ...
      >/
      <router-link to="/jurusan" class="capitalize text-blue-500"
        >jurusan</router-link
      >/
    </div>
    <div>
    ...
    <table class="my-4 table-auto w-full text-center">
      ...
      <tbody>
        <tr
          ...
          :key="major.id"
        >
          <td class="text-xs lg:text-base">{{ major.major }}</td>
          <td class="flex gap-2 justify-center items-center h-14">
            <router-link
              :to="'/edit-jurusan/' + major.id"
            ...
            ></router-link>
            <form @submit.prevent="destroy(major.id)">
              <button
